...
def load_args(default_config=None):
    ...
    parser.add_argument('--video-direc', default="/root/autodl-tmp/data/mlavt_tedx/es", help='raw video directory')
    parser.add_argument('--landmark-direc', default="/root/autodl-tmp/data/mlavt_tedx/es/landmark", help='landmark directory')
    parser.add_argument('--filename-path', default="/root/autodl-tmp/data/mlavt_tedx/es/file.list", help='list of detected video and its subject ID')
    parser.add_argument('--save-direc', default="/root/autodl-tmp/data/mlavt_tedx/es/video_fxxk", help='the directory of saving mouth ROIs')
    # -- mean face utils
    parser.add_argument('--mean-face', default="/root/autodl-tmp/model/av_hubert/avhubert/checkpoints/20words_mean_face.npy",
                        type=str, help='reference mean face (download from: https://github.com/mpc001/Lipreading_using_Temporal_Convolutional_Networks/blob/master/preprocessing/20words_mean_face.npy)')
    # -- mouthROIs utils
    ...
    parser.add_argument('--ffmpeg', default="/usr/bin/ffmpeg", type=str, help='ffmpeg path')
    parser.add_argument('--task', type=str, default='en', help='src or en')
    parser.add_argument('--rank', default="0", type=int, help='rank id')
    parser.add_argument('--nshard', default="1", type=int, help='number of shards')
...
if __name__ == '__main__':
    ...
    for filename_idx, filename in enumerate(tqdm(fids)):
        video_pathname = os.path.join(args.video_direc, filename, f'{args.task}.avi')

        landmarks_pathname = os.path.join(args.landmark_direc, filename, '00.pkl')
        dst_pathname = os.path.join(args.save_direc, filename, '00.avi')
