...
def main():
    ...
    parser.add_argument('--lrs3', default="/root/autodl-tmp/data/mlavt_tedx/es", type=str, help='lrs3 root dir')
    # parser.add_argument('--valid-ids', required=False, type=str, help='a list of valid ids')
    parser.add_argument('--vocab-size', type=int, default=1000, help='a list of valid ids')
    args = parser.parse_args()
    file_list, label_list = f"{args.lrs3}/file.list", f"{args.lrs3}/label_en.list"
    ...
    vocab_dir = (Path(f"{args.lrs3}")/f"spm{vocab_size}_en").absolute()
        ...
        for name, data in zip(['train', 'valid', 'test'], [train, valid, test]):
            with open(f"{target_dir}/{name}.tsv", 'w') as fo:
                ...
                for fid, _, nf_audio, nf_video in data:
                    fo.write('\t'.join([fid, os.path.abspath(f"{video_dir}/{fid}/00.avi"), os.path.abspath(f"{audio_dir}/{fid}/00.wav"), str(nf_video), str(nf_audio)])+'\n')
    ...
    valid_fids = set([x.strip() for x in open(f"{args.lrs3}/valid.txt").readlines()])
    test_fids = set([x.strip() for x in open(f"{args.lrs3}/test.txt").readlines()])
    train_fids = set([x.strip() for x in open(f"{args.lrs3}/train.txt").readlines()])
    train, valid, test = [], [], []
    for fid, label, nf_audio, nf_video in zip(fids, labels, nfs_audio, nfs_video):
        if fid in test_fids:
            test.append([fid, label, nf_audio, nf_video])
        elif fid in valid_fids:
            valid.append([fid, label, nf_audio, nf_video])
        else:
            train.append([fid, label, nf_audio, nf_video])

    dir_200h = f"{args.lrs3}/200h_data_en"
    print(f"Set up 200h dir")
    os.makedirs(dir_200h, exist_ok=True)
    setup_target(dir_200h, train, valid, test)
    return
