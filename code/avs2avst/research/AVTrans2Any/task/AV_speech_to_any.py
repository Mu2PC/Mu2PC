...
from research.AVTranslation.dataset.AV_speech_to_text_dataset import AVSpeechToTextDatasetCreator, AVS2SDataConfig
from research.AVTranslation.task.sequence_generator import SequenceGenerator
logger = logging.getLogger(__name__)


@register_task("AV_speech_to_text")
class AVSpeechToTextTask(LegacyFairseqTask):
    @classmethod
    def add_args(cls, parser):
        ...
        parser.add_argument("--noise_fn", type=str, default=None)
        parser.add_argument("--noise_prob", type=float, default=None)
        parser.add_argument("--noise_snr", type=float, default=None)
        parser.add_argument("--modalities", type=str, choices=["av", "v", "a"], default=None)
        parser.add_argument("--no_image_aug", action="store_true", default=None)

    def __init__(self, args, tgt_dict):
        super().__init__(args)
        self.cfg = args
        self.tgt_dict = tgt_dict
        self.data_cfg = AVS2SDataConfig(Path(args.data) / args.config_yaml)

        # Override
        if args.modalities is not None:
            dict = {'av': ["video", "audio"], 'v': ["video"], 'a': ["audio"]}
            self.data_cfg.config["modalities"] = dict[args.modalities]
        if args.no_image_aug is not None:
            self.data_cfg.config["image_aug"] = False
        for key in ['noise_fn', 'noise_prob', 'noise_snr']:
            if getattr(args, key) is not None:
                self.data_cfg.config[key] = getattr(args, key)
    ...
    def setup_task(cls, args, **kwargs):
        data_cfg = AVS2SDataConfig(Path(args.data) / args.config_yaml)
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = AVSpeechToTextDatasetCreator.from_tsv(
            self.args.data,
            self.data_cfg,
            split,
            self.tgt_dict,
    ...
    def build_model(self, args, from_checkpoint=False):
        ...
        model = super(AVSpeechToTextTask, self).build_model(args, from_checkpoint)
        self.generator = None
