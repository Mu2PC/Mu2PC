...
class AVHubertDataset(FairseqDataset):
    ...
    def load_feature(self, mix_name):
        ...
        if 'video' in self.modalities: # 读取video
            video_feats = self.load_video(video_fn) # [T, H, W, 1]
        else:
            video_feats = None
        if 'audio' in self.modalities: # 读取audio
            audio_fn = audio_fn.split(':')[0]
            sample_rate, wav_data = wavfile.read(audio_fn)
            assert sample_rate == 16_000 and len(wav_data.shape) == 1
            if np.random.rand() < self.noise_prob: # 假如需要加入noise
                wav_data = self.add_noise(wav_data)
            audio_feats = logfbank(wav_data, samplerate=sample_rate).astype(np.float32) # [T, F] 提取频谱
            audio_feats = stacker(audio_feats, self.stack_order_audio) # [T/stack_order_audio, F*stack_order_audio] 修改维度
        else:
            audio_feats = None
        if audio_feats is not None and video_feats is not None: # 对齐两者长度
    ...
    def select_noise(self): # 随机选择noise加上
    ...
    def add_noise(self, clean_wav): # mix clean audio & noise sample，同时注意adjust两者量级
    ...
    def __getitem__(self, index):
        ...
        if self.normalize and 'audio' in self.modalities: # normalize 频谱
            with torch.no_grad():
    ...
    def collater(self, samples):
        ...
        if audio_source is not None:
            ...
            audio_sizes = [len(s) for s in video_source] # 若不存在audio_source，则将size其用video size代替
        if self.pad_audio:
            audio_size = min(max(audio_sizes), self.max_sample_size) # pad到最长音频长度
        else:
            audio_size = min(min(audio_sizes), self.max_sample_size) # 裁切到最短音频长度
        if audio_source is not None:
        ...
        if video_source is not None:
            collated_videos, padding_mask, audio_starts = self.collater_audio(video_source, audio_size, audio_starts) # 同时需要确保video start与audio start一致
        else:
        ...
        net_input = {"source": source, "padding_mask": padding_mask} # padding_mask为audio/video的mask
        batch = {
        ...
        if self.single_target: # finetune时，只有1个target seq
            batch["target_lengths"] = lengths_list[0]
            batch["ntokens"] = ntokens_list[0]
            if self.is_s2s: # 若为seq-to-seq model
