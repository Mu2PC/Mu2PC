...
class Trainer(object):
    ...
    def is_moe(self):
        return (
            ...
            or getattr(self.cfg.model, "encoder_moe_freq", 0) > 0
            or getattr(self.cfg.model, "decoder_moe_freq", 0) > 0
            or getattr(self.cfg.model, "base_layers", 0) > 0
        )
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            if (
                ...
                # if self.is_moe:
                #     state_all = checkpoint_utils.load_checkpoint_to_cpu(
                #         filename.split("-rank-")[0] + "-rank-0-shard0.pt",
                #         is_moe=True,
                #     )
                #     for key in state_all["model"]:
                #         if key not in state["model"]:
                #             state["model"][key] = state_all["model"][key]
