...
def wav2lip_one_video(pair):
	# /root/autodl-tmp/data/mtedx/tmp/es/pycrop/_68m86wqV0w/000354_000382/00.avi
	# /root/autodl-tmp/data/mtedx/tts/es_en/train/_pew7s6sagc/002078_002151.wav
	face_in, audio_in = pair
	yt_id, utt_id = "/".join(face_in.rsplit('/', 3)[-3:]).split('/', 1)
	utt_id = "_".join(utt_id.split('/'))
	outfile_name = "/".join([yt_id, utt_id])
...
def main():
	...
	for clip_id in wav2lip_id:
		# XzfmkvWAs-k/015343_015398/00.txt
		yt_id, utt_id, utt_num = clip_id.split('.')[0].split('/')
		face_path = os.path.join(video_root, clip_id.split('.')[0] + ".avi")
		wav_path = os.path.join(tts_root, yt_id, utt_id + ".wav")
...
if __name__ == '__main__':
	...
	# /root/autodl-tmp/data/mtedx/tmp/es/pycrop/_68m86wqV0w/000354_000382/00.avi
	# /root/autodl-tmp/data/mtedx/tts/es_en/train/_pew7s6sagc/002078_002151.wav
	video_root = f"/root/autodl-tmp/data/mtedx/tmp/{src_lang}/pycrop"
	tts_root = f"/root/autodl-tmp/data/mtedx/tts/{src_lang}_en/train"
	out_root = f"/root/autodl-tmp/data/mtedx/target_video/{src_lang}"
	os.makedirs(out_root, exist_ok=True)
	# manifest_path = f"/root/autodl-tmp/data/mtedx/target_video/manifest_{src_lang}.txt"
	manifest_path = f'/root/autodl-tmp/data/mtedx/manifest/{src_lang}2en/manifest_src.txt'

	error_file = open(f'/root/autodl-tmp/data/mtedx/target_video/{src_lang}_errors.txt', 'w')
