...
class AVHubertSeq2Seq(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, cfg, task):
        ...
        if cfg.w2v_args is None: # 如果load pretrained model
            state = checkpoint_utils.load_checkpoint_to_cpu(
                cfg.w2v_path, arg_overrides
            )
        ...
        if state is not None and not cfg.no_pretrained_weights:
            ...
            encoder.w2v_model.load_state_dict(state["model"], strict=False)  # encoder读取state dict
    ...
    def forward(self, **kwargs):
        ...
        kwargs:
            ...
            padding_mask: [B, T1]
            prev_output_tokens: [B, T2]
        '''
        ft = self.freeze_finetune_updates <= self.num_updates   # 前N步fix参数
        with torch.no_grad() if not ft else contextlib.ExitStack():
